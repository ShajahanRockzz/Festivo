...
@Component({
  ...
  templateUrl: './participanthome.html',
  styleUrl: './participanthome.scss',
})
export class Participanthome implements OnInit {
  fests: Fest[] = [];
  isLoading = true;
  errorMessage = '';
  searchTerm = '';
  ...
  loadCategories(): void {
    this.http.get('/api/category/all').subscribe({
      next: (response: any) => {
        ...
        const catTrc: string[] = Array.from<string>(
          new Set<string>(rawCategories.map((c: any) => (c || '').trim().toLowerCase()))
