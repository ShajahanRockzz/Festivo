...
const htmlContent = 
<section class="viewallfests">
  <div class="page-header">
    <div>
      <h1 class="premium-title">Available Fests</h1>
      <p class="premium-subtitle">Explore fests that are active, started, and hosted by subscribed institutions.</p>
    </div>
  ...
  <div class="filters-bar floating-dock" *ngIf="!isLoading">
    <div class="search-field">
      <i class="feather icon-search search-icon"></i>
      <input
        type="text"
        placeholder="Search fests by name or institution..."
        [(ngModel)]="searchTerm"
      />
    </div>
    <div class="select-group">
      <div class="filter-wrapper" [class.active]="selectedCategory === 'all'">
      ...
      <div class="filter-wrapper" [class.active]="selectedAudience === 'all'">
        <select [(ngModel)]="selectedAudience">
  ...
  <div *ngIf="isLoading" class="loading-state glass-panel">
    ...
    <p>Discovering available fests...</p>
  </div>

  <div *ngIf="!isLoading && errorMessage" class="error-state glass-panel">
    <p><i class="feather icon-alert-triangle"></i> {{ errorMessage }}</p>
  </div>

  <div *ngIf="!isLoading && !errorMessage && fests.length === 0" class="empty-state glass-panel">
    <div class="icon-wrap"><i class="feather icon-calendar"></i></div>
    <p>No available fests right now. Please check back later.</p>
  </div>

  <div *ngIf="!isLoading && !errorMessage && fests.length > 0 && filteredFests.length === 0" class="empty-state glass-panel">
    <div class="icon-wrap"><i class="feather icon-search"></i></div>
    <p>No fests match your current criteria. Try adjusting your filters.</p>
  </div>

  <!-- Bento Grid Architecture -->
  <div *ngIf="!isLoading && filteredFests.length > 0" class="bento-grid">
    <div class="floating-card fest-card" *ngFor="let fest of filteredFests; let i = index" [style.--animation-order]="i">
      ...
      <div class="fest-image-wrapper">
        <img [src]="getImageUrl(fest.fest_image)" [alt]="fest.fest_name" class="fest-image" />
        <div class="image-mask"></div>
        <span class="fest-category" *ngIf="fest.category_name">{{ fest.category_name }}</span>
      </div>
      ...
      <div class="fest-info">
        ...
        <div class="bento-details">
          ...
          <div class="detail-item" *ngIf="fest.fest_for">
            <span class="meta-label">EVENTS</span>
            <span class="meta-value">{{ fest.fest_for }}</span>
          </div>
        </div>
        
        <button class="btn-ghost-glow">View Details</button>
      </div>
    </div>
...
fs.writeFileSync('src/app/Participant/viewallfests/viewallfests.html', htmlContent);
fs.writeFileSync('src/app/Participant/viewallfests/viewallfests.scss', scssContent);
